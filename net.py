...
class ImpactNet:
    ...
    def __init__(self, M, P, n, learning_rate, length, hidden_layer_size, training_data=None):
        ...
        self.n = n # on average, number grids per image (M*n ~ downscaled size of the image)
    ...
    def train(self, logging=False):
        ...
        for i in range(len(self.training_data['values'])):
            results.append(self.evaluate(self.training_data['values'][i])) # evaluate net on current training example
        print('Evaluate training examples, beggining gradient descent')
        resultCounter = 0
        ...
        for l in range(1, len(self.weights)):
            for k in range(len(self.weights[l])):
                for j in range(len(self.weights[l][k])):
                    ...
                    self.weights[l][k][j] = self.weights[l][k][j] + self.weights[l][k][j] * -der_w
    ...
    def backprop(self, l, k, l_w, k_w, j_w, results, depth=0):
        if l - l_w > 1:
            for j in range(self.layer_sizes[l-1]):
                ...
                sum_derivatives_a_l_minus_1 = (
                    ...
                    self.backprop(l - 1, j, l_w, k_w, j_w, results, depth + 1) # * da_l-1/dw
                )
    ...
    def evaluate(self, training_example):
...
if __name__ == '__main__':
    net = ImpactNet(M, P, n, learning_rate, length, hidden_layer_size, u.get_image_data())
    net.train(True)
    pickle.dump(net.weights, open('trained_net_weights.pickle', 'wb'))
    pickle.dump(net, open('trained_net.pickle', 'wb'))
else:
    tn = ImpactNet(M, P, n, learning_rate, length, hidden_layer_size, u.random_inputs(u.initialise_layer_sizes(M, length, hidden_layer_size), n))
    print(f'Test network tn created with args: (M={M}, P={P}, n={n}, learning_rate={learning_rate}, length={length}, hidden_layer_size={hidden_layer_size}, training_data=RANDOM_INPUTS)')